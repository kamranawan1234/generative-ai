...
public class PopulationPanel extends JPanel {
    ...
    public void highlightMutations(int individualIndex, boolean[] mutationMask) {
        for (int j = 0; j < mutationMask.length; j++) {
            if (mutationMask[j])
                mutationPoints.add(new Point(individualIndex, j));
        }
        repaint();
    ...
    public void highlightCrossover(int individualIndex, boolean[] crossoverMask) {
        for (int j = 0; j < crossoverMask.length; j++) {
            if (crossoverMask[j])
                crossoverPoints.add(new Point(individualIndex, j));
        }
        repaint();
        new javax.swing.Timer(500, e -> {
            for (int j = 0; j < crossoverMask.length; j++)
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            g2.drawRect(xOffset + 1, yOffset + i * cellSize + 1, numCols * cellSize - 2*1, cellSize - 2*1);
                ...
                g2.drawString(label, xOffset + numCols * cellSize + 10, yOffset + i * cellSize + cellSize - 10);
            }
        }
    }
